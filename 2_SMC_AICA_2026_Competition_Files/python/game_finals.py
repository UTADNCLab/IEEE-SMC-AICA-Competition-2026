...
# curb penalty
# -------------------------------------------------------------------------
# The QCar2 is expected to remain on the road surface. Moving freely within
# the road is not penalised; leaving it is.
#
# Detection is positional, not motion-based. The car's four wheel positions
# are tested against a fixed occupancy grid of the drivable area, so the
# rule is a geometric fact -- a wheel is on the road surface or it is not --
# rather than an inference from how the vehicle moved.
...
# Continuous contact is charged repeatedly: one further hit for every
# curb_REPEAT_TIME seconds the vehicle remains off the road. Without this,
# a sustained excursion along a curb would cost exactly the same as a
# momentary clip, which does not reflect the difference in driving.
...
# -------------------------------------------------------------------------
# Occupancy-grid curb detection
# -------------------------------------------------------------------------
# drivable_grid.npz is generated by build_drivable_grid.py from the SDCS
# mat texture, using the simulator's own pixel-to-world transform. The grid
# is a fixed artefact: it is identical for every team and every run, and
# contains no fitted or tuned geometry.
...
#
#   curb_MIN_DEPTH  distance past the road edge a wheel must reach. The
#                   grid stores this distance for every off-road cell, so
#                   the value is measured rather than inferred. At the
#                   current grid resolution of 0.10 m this is the smallest
#                   representable value, i.e. any crossing qualifies.
#   curb_MIN_TICKS  consecutive 10 Hz samples the condition must hold.
#                   Set to 1: at 10 Hz a brief contact can occupy a single
#                   sample, and requiring two produced inconsistent results
#                   for identical driving lines. The one-hit-per-excursion
#                   latch prevents a single contact being counted twice.
curb_MIN_DEPTH  = 0.10   # m past the road edge before it counts
curb_MIN_TICKS  = 1      # consecutive qualifying ticks before it counts

# Writes curb_trace.csv: position, heading, wheels off the road and contact
# depth for every tick of the run. plot_trace.py renders this as the
# vehicle's path over the drivable area with each scored hit marked,
# providing a visual record of where and why each penalty was applied.
curb_TRACE      = True

# -------------------------------------------------------------------------
# Each hit prints a single line at the moment it is recorded. The score
# displayed during the run is the raw delivery score; the curb penalty is
# applied once at the end. Every hit is logged with its time, coordinates
# and depth, and the full summary -- hit list, total, deduction and final
# score -- is printed when the run ends.

# Deliveries completed after this many seconds (5 min) add NOTHING to the
# score at all -- not even a floor amount. Before this cutoff, score is
# still CONSTANT_SCORE - current_time (+ any bonus), same as always.
...
# The "urgent" window pays an extra early-bird bonus if delivered within
# the first urgent_WINDOW_TIME_LIMIT seconds. Missing it forfeits
# nothing -- the delivery still pays its normal window bonus any time
# later, it just loses the extra. The colour cue lives on the PAD only
# (setup_env_finals.py's floor marker); the completion box spawns the
# normal default colour like every other delivery.
...
# -------------------------------------------------------------------------
# Buildings -- one uniform rule, no special cases
# -------------------------------------------------------------------------
# A building owns N delivery SLOTS. Every slot is either/or: fill it from
# the building's ground pad OR from any one of its window pads. Nothing is
# locked to a vehicle -- the car can do a whole building on the ground, the
# drone can do the same building entirely through its windows, or any mix.
# The only drone-exclusive buildings are the ones with no ground pad at all
# (Building 6 and 7, the solo windows).
#
# A window pad takes at most one box and pays a bonus on top of the base
# score. The ground pad takes as many boxes as the building still has free
# slots. Once all N slots are filled the building is CLOSED -- further
# drops at any of its pads score nothing -- and every pad it owns lights
# up at once, including ones nobody delivered to.
#
#   "slots"   -- deliveries required, and the maximum accepted
#   "ground"  -- ground/common pad coordinate, or None for window-only
#   "windows" -- window pads: display label, coordinate, bonus
BUILDINGS = [
...
def reveal_building(shape, b, building_filled, window_used,
    ...
    """
    Light up a building's pads once all its slots are filled -- ALL of
    them, ground and every window, including pads nobody delivered to,
    because the building is done as a whole.

    Below that nothing shows, with one exception: past the 5-minute score
    cap the building can never be finished, so whatever IS done gets
    revealed rather than staying hidden forever.

    Idempotent -- safe to call on every delivery and once a second from
    the main loop.
    """
    spec = BUILDINGS[b]
...
def spawn_window_pad(shape, wi, window_spawned):
    """A window's own pad shows the moment it's delivered to -- windows
    never wait for the rest of their building."""
    if window_spawned[wi] == 1:
        return
    window_spawned[wi] = 1
    spawn_box(shape, actor_number=230 + wi,
...
class DrivableGrid:
    """Occupancy grid of the drivable road surface.

    Loaded from drivable_grid.npz, generated offline by
    build_drivable_grid.py from the SDCS mat texture in the simulator's own
    pixel-to-world transform. Holds two arrays:

        mask    True where the surface is drivable
        depth   distance past the road edge, in metres, for every
                off-road cell; zero on the road

    If the file is absent the run proceeds with curb detection disabled
    rather than failing, so a missing artefact cannot halt a competition
    run.
    """
    ...
    def wheels_state(self, x_rear, y_rear, yaw):
        """Return (wheels off the road, depth of the furthest one).

        Wheel positions are derived from the vehicle pose: rear axle at
        (x_rear, y_rear), front axle one wheelbase ahead, each axle
        CAR_HALF_TRACK either side of centreline. Depth is read from the
        grid's distance field, giving how far past the road edge the
        furthest wheel has travelled.
        """
        if not self.ok:
            return 0, 0.0
...
def main():
    ...
    # Drop-hold timers. The car and the drone require separate ground
    # timers: both are evaluated every tick, and whichever is not currently
    # dropping resets its own array. A shared timer would be cleared by one
    # vehicle while the other was mid-drop, preventing completion.
    ...
    # How many of each building's slots are filled, and which window pads
    # have been used (a window pad takes one box only).
    ...
    # curb penalty state (see the thresholds above). max_wheels_off,
    # max_depth, near_misses and shallow_ticks are accumulated for
    # diagnostic use; they do not affect scoring.
    curb_hits = 0
    ...
    try:
        while timer.check():
            ...
            if off >= curb_MIN_WHEELS and depth >= curb_MIN_DEPTH:
                ...
                # Contact that did not meet the thresholds, tallied for the
                # end-of-run summary. Large values indicate the thresholds
                # are excluding genuine contact.
            ...
            if struck:
                ...
                # Back on the road. Re-arm only once the vehicle has been
                # clear for HIT_CLEAR_TIME AND has travelled HIT_CLEAR_DIST,
                # so neither a wheel oscillating on the boundary nor the
                # rear axle crossing the front axle's ground records again.
            ...
            # End of run
            # =========================================================
            # Nothing scores after SCORE_TIME_CAP, so that IS the end of the
            # run -- stop there rather than idling on to SIMULATION_TIME.
            # Reveal whatever the buildings managed first, so the partial
            # reveals are actually visible before the loop exits.
            if current_time >= SCORE_TIME_CAP:
        ...
        # A slot is filled either at the ground pad or at a window pad;
        # window_used says which ones went through a window, so the rest of
        # the filled slots were ground drops.
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print()
    # Hold the console open at the end -- otherwise the window closes the
    # instant the run finishes and the final score scrolls away unseen.
    try:
        main()
    except Exception:
        import traceback
